const BASE = '' // Vite dev proxy forwards /api and /health to the backend
...
export const api = {
  ...
  reverseGeocode: (lat, lon) => request(`/api/geocode?lat=${lat}&lon=${lon}`),
  ...
  transitStops: (lat, lon, radiusM = 800) =>
    request(`/api/transit-stops?lat=${lat}&lon=${lon}&radius_m=${radiusM}`),
  searchJourneys: (payload) =>
    request('/api/journeys/search', {
      method: 'POST',
      body: JSON.stringify(payload),
    }),
  getJourney: (id) => request(`/api/journeys/${id}`),
}
